...
def main():
    ...
    with open('../data/src.csv', 'rb') as csvfile:
        ...
        for row in source_file:
            a = d1[row[0]][row[1].split('-')[0]]
            i_a = float(a) if a else -1000
            i_b = float(row[2]) if row[2] else -1000
            if i_a < i_b or not a:
    ...
    with open('../data/result.csv', 'wb') as f:
        ...
        for key in l2:
            try:
                writer.writerow([key[0], key[1], d1[key[0].zfill(6)][key[1]]])
            except:
